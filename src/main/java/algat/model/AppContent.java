...
public class AppContent {
    ...
    public AppContent(String contentPath) throws IOException {
        ...
        InputStream inStream = App.class.getResourceAsStream(contentPath + "/" + ORDER_FILE_EXPECTED_NAME);           // Assumes that the App class stays at the root of the classpath
        if (inStream != null) {
            ...
            while (topicFolderName != null) {
                topicList.add(new Topic(topicFolderName, contentPath + "/" + topicFolderName));
                topicFolderName = br.readLine();
            }
            br.close();
        } else {
            System.out.println("ERROR: it was impossible to find the resource [" + contentPath + "/" + ORDER_FILE_EXPECTED_NAME + "] during data AppContent contruction");
            exit(1);
        }
